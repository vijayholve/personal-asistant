...
def retry_recognition(recognizer, source):
    """
    Provides a chance to retry speech recognition if an error occurs.

    Returns the recognized text (lowercased) or None if unsuccessful.
    """
    print("Recognition failed. Do you want to retry? (y/n)")
    response = input("> ").lower()
    if response == 'y':
        return improved_speech_recognition()
    else:
        print("Speech recognition cancelled.")
        return None

# Example usage
text = improved_speech_recognition()
if text:
