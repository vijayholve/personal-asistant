...
def main(text):
    ...
    open=["open","Open"]
    messgae=["messages","massage","message","Messages","Massage","Message"]
    ...
    try:
        if any(o in text for o in open):
            ...
            if "to" in text:
                ...
                person = parts[1].strip()lo
                
                tts(f"Message is {msg_part} to send to {person}")
                for p in per_name:
                    if p.name.lower in person or p.name.lower is person:
                        pid=p.id
                        print("id is sended : ",pid)
                        open_whatsapp(pid, msg_part)
